...
describe("[DELETE] /api/v1/tokens/delete-signup-token/:tokenId", async () => {
  it("returns a 401 when user tries to delete token she/he didn't generate", async () => {
    ...
    return request(app)
      .delete(`/api/v1/tokens/delete-signup-token/${token.tokenId}`)
  ...
  it("returns a 404 if no token is found", async () => {
    ...
    return request(app)
      .delete(`/api/v1/tokens/delete-signup-token/${"tokenIdDoesNotExist"}`)
      .set("Authorization", `Bearer ${res.body.accessToken}`)
      .expect(404);
  });
  ...
  it("returns a 200 on successful deleting", async () => {
    const newUser = await prisma.user.create({
      data: {
        firstName: "John",
        lastName: "Doe",
